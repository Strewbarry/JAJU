...
class dijkstra_path_pub :
    def __init__(self):
        ...
        self.latitude = 37.24316834
        self.longitude = 126.77527165
        ...
        self.ego_x, self.ego_y = None, None
        self.is_gps=False

        self.proj_UTM = Proj(proj='utm', zone=52, ellps = 'WGS84', preserve_units=False)

        self.utm_x, self.utm_y, self.zone_number, self.zone_letter = self.trans_gps_to_utm()
        ...
        while True:
            if self.is_goal_pose == True and self.is_init_pose == True:
                rospy.loginfo('makepath complete')
                break
            else:
    ...
    def trans_gps_to_utm(self):
        utm_x, utm_y, zone_number, zone_letter = utm.from_latlon(self.latitude, self.longitude)
    ...
    def convertLL2UTM(self):
        ...
        if self.lon == 0 and self.lat == 0:
            ...
            self.ego_x = xy_zone[0] - self.e_o
            self.ego_y = xy_zone[1] - self.n_o
        rospy.loginfo('convert complete')
    ...
    def init_callback(self):
        ...
        self.init_x = self.ego_x
        self.init_y = self.ego_y
        rospy.loginfo('startpoint detected')
    ...
    def goal_callback(self):
        ...
        self.goal_x = self.utm_x
        self.goal_y = self.utm_y
...
# 15:52 현재지점 부터 하드코딩 목표지점까지 경로생성 완료