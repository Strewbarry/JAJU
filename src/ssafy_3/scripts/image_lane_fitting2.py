...
class CURVEFit:    
    def __init__(self, order, alpha, lane_width, y_margin, x_range, dx, min_pts):
        ...
        self.ransac_left = linear_model.RANSACRegressor(estimator=linear_model.Lasso(alpha=self.alpha),
                                                        max_trials=100,
                                                        loss='absolute_loss',
                                                        min_samples=self.min_pts,
                                                        residual_threshold=self.y_margin)

        self.ransac_right = linear_model.RANSACRegressor(estimator=linear_model.Lasso(alpha=self.alpha),
                                                        max_trials=100,
                                                        loss='absolute_loss',
                                                        min_samples=self.min_pts,
                                                        residual_threshold=self.y_margin)
